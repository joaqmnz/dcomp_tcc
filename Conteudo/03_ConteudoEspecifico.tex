...
\begin{quadro}[H]
    ...
    \begin{tabular}{|>{\raggedright\arraybackslash}p{4cm}|
        ...
        \hline
        Visualização de TAF pelo Avaliador & Avaliador visualiza outros TAFs, mas não visualiza os Grupos dos mesmos & Avaliador & \textit{Web} \\
        \hline
    \end{tabular}
\end{quadro}
...
Para este trabalho, foi necessário envolver a API em Python, além de inserir novas opções no \textit{Frontend} para a inserção de arquivos CSV no cadastro de Militares, adotando algumas estratégias para sinalizar ao usuário qual tipo de arquivo estava sendo utilizado naquele momento da criação. Uma dessas estratégias consiste em modifiar o tipo do arquivo em todos os textos presentes na funcionalidade, como mostra a \autoref{fig:importar_militares_excel} para a funcionalidade Excel e a \autoref{fig:importar_militares_csv} para a funcionalidade CSV. Uma outra estratégia é permitir que o usuário realize o \textit{download} de um arquivo exemplo para preenchimento das informações.
